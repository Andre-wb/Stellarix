...
Stellarix — **настольное приложение** (Tauri). Веб-развёртывания у проекта нет: акустический
модем целиком реализован нативными командами Tauri поверх аудиоустройств системы, поэтому
в обычном браузере передача звуком не работает и не поддерживается. Страницы приложения
обслуживает локальный сервер на `127.0.0.1:8000`, но открывать этот адрес в браузере
бессмысленно — аудио-интерфейс там заблокирован с соответствующим предупреждением.

PostgreSQL приложение запускает **встроенный** (порт 15432) и само создаёт секреты в своём
каталоге данных — ставить и настраивать базу вручную для запуска не требуется.
...
Файл `.env` создавать не нужно. При запуске приложение само генерирует `USERNAME_SECRET`
и `SESSION_SECRET` в приватном каталоге данных и подставляет `DATABASE_URL` встроенной
базы. Разделы выше про установку и настройку PostgreSQL вручную нужны только тем, кто
хочет работать с базой напрямую (`psql`, миграции `sqlx`), — для обычного запуска они
необязательны.
...
4. Проверьте на занятость порты приложения: `netstat -ano | findstr :8000` и `netstat -ano | findstr :15432`
5. Если приложение жалуется на занятый порт — закройте второй экземпляр Stellarix или процесс, который держит эти порты
